...
function setupBoard()
	...
	for p, peice in ipairs(Piecelist) do -- place peices
		for i, square in ipairs(board.squarelist) do
			if peice.location.column == square.id.column and peice.location.row == square.id.row then
				peice.x = square.x
				peice.y = square.y
...
function isUnderAttack(square)
	...
	for i, piece in ipairs(Piecelist) do
		if piece.color ~= playerTurn and piece.type ~= "king" then --[[temp. will need to check king squares eventually
			...
			for i, move in ipairs(piece:getValidMoves()) do
				if move.column == square.id.column and move.row == square.id.row then
					return true
				end
			end
		end
